...
import headerList from './itemList';
...
const Sidebar = () => {
  ...
  const itemList = headerList(theme.iconColor)
  return (
    <List
      component="nav"
          ...
          <React.Fragment key={index}>
            <ListItem
              ...
              <span
                ...
                <ListItemIcon className='nav__icon' style={{ color: router.pathname === `/dashboard/${item.path}` ? "#fff" : theme.textColor }}>
                  {item.icon}
                </ListItemIcon>
                {!minimize && <ListItemText className="nav_item_text" style={{width: '130px'}} primary={item.title} />}
              </span>
                  ...
                  <List component="div" disablePadding>
                    <NavLink style={{ color: theme.textColor }} className="d-flex sidebar nav__link" to={{ pathname: childItem.path }}>
                      <ListItem button className={classes.nested}>
                        <ListItemIcon
                          style={{ color: router.pathname === `/dashboard/${item.path}` ? "#fff" : theme.iconColor }}>
                          {childItem.icon}
                        </ListItemIcon>
